...
DATABASE_URL = (
    "postgresql://neondb_owner:npg_1YgAylzvPw9x"
    "@ep-lively-shadow-a4o9mzps-pooler.us-east-1.aws.neon.tech/"
...
# —————— Instancia Flask a nivel de módulo ——————
app = Flask(__name__)
...
def clientes():
    ...
    if cliente_id:
        ...
        # Último pedido o crear uno nuevo
        cur.execute("""
        ...
        # Detalle del pedido
        cur.execute("""
          SELECT p.id_producto, p.descripcion_producto, p.precio_unitario,
                 dp.cantidad_productos,
                 (dp.cantidad_productos * p.precio_unitario) AS total_linea
...
def add():
    ...
    cur.execute("""
      ...
      DO UPDATE SET cantidad_productos =
...
def factura_final():
    ...
    # Datos del cliente
    cur.execute("""
      SELECT c.nombre_cte, c.rfc_cte, c.direccion_cte,
             e.nombre_estado, ci.ciudad_cte
      FROM public.cliente c
      JOIN public.estado e   ON c.id_estado = e.id_estado
      JOIN public.ciudades ci ON c.id_ciudad = ci.id_ciudad
    ...
    # Detalles finales
    cur.execute("""
      SELECT p.descripcion_producto, dp.cantidad_productos,
             p.precio_unitario,
             (dp.cantidad_productos * p.precio_unitario) AS total_linea
      FROM public.detalle_pedido dp
      JOIN public.producto p ON dp.id_producto = p.id_producto
